...
description: >-
  ...
  and run git on this Windows/Git Bash repo. Use when committing changes,
...
How to ship changes to the live AnyChar host and handle the Windows/Git Bash
git gotchas.
...
- `run_command` always executes via Git Bash, so `&&` chaining works normally
  (unlike PowerShell, which needs `;`). Just be aware if a *human* on this repo
  is running PowerShell instead, they'll need `;` chaining.
- Multiline commit messages: writing a heredoc inline can be flaky across
  shells. Prefer writing the message to a temp file and using `git commit -F`:
  ```bash
  # write message to .git/COMMIT_EDITMSG_TMP, then:
  git commit -F .git/COMMIT_EDITMSG_TMP && rm .git/COMMIT_EDITMSG_TMP
  ```
- Never use interactive flags (`git rebase -i`, `git add -i`) — they hang the
  agent terminal (no TTY to drive them).
