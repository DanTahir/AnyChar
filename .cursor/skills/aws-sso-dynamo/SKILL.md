...
AWS CLI v2 pipes output through a pager (`more` on Windows). In the agent
terminal that pager **blocks forever** and wedges the whole shell. Every
`aws` command MUST disable it:

- Always pass `--no-cli-pager`, AND
- Set `$env:AWS_PAGER=""` (PowerShell) / `export AWS_PAGER=""` (bash) at the
  start of the session.

If the shell ever hangs with "no exit status" after an `aws` call, the pager
got you — kill `more.com`/`aws.exe` (Task Manager or `taskkill //IM more.com //F`
in Git Bash) and restart the terminal.
...
PowerShell needs `\"` escaping inside the JSON values arg. All examples assume
`$env:AWS_PAGER=""` was set.
...
the full item you care about. From Git Bash, drop the backticks/`\"` and use
single quotes around the JSON instead.
